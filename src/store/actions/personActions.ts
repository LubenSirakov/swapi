...
import { PersonAction, PersonData, PersonError, GET_PERSON, SET_LOADING, SET_ERROR } from '../types';

const getRandomNumber = (min: number, max: number) => {
    return Math.random() * (max - min) + min;
}

export const getPerson = (name: string): ThunkAction<void, RootState, null, PersonAction> => {
...
export const getrandomPerson = (name: string): ThunkAction<void, RootState, null, PersonAction> => {
    return async dispatch => {
        try {
